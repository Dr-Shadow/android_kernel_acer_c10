...
{
	mt_lbprof_printf(MT_LBPROF_TASK, "%d:pull:%d:%d:%s\n", env->dst_cpu, env->src_cpu, p->pid, p->comm);
...
int can_migrate_task(struct task_struct *p, struct lb_env *env)
{
	...
	if (!cpumask_test_cpu(env->dst_cpu, tsk_cpus_allowed(p))) {
		...
		if(mt_lbprof_lt (env->sd->mt_lbprof_nr_balance_failed, MT_LBPROF_NR_BALANCED_FAILED_UPPER_BOUND))
			mt_lbprof_printf(MT_LBPROF_LBLOG, "%d:balance fail:affinity:%d:%d:%s:0x%lu\n", env->dst_cpu, env->src_cpu, p->pid, p->comm, p->cpus_allowed.bits[0]);
		return 0;
	}
	...
	if (task_running(env->src_rq, p)) {
		...
		if( mt_lbprof_lt (env->sd->mt_lbprof_nr_balance_failed, MT_LBPROF_NR_BALANCED_FAILED_UPPER_BOUND))
			mt_lbprof_printf(MT_LBPROF_LBLOG, "%d:balance fail:running:%d:%d:%s\n", env->dst_cpu, env->src_cpu, p->pid, p->comm);
		return 0;
	}
	...
	if (tsk_cache_hot) {
		...
		if(mt_lbprof_lt (env->sd->mt_lbprof_nr_balance_failed, MT_LBPROF_NR_BALANCED_FAILED_UPPER_BOUND))
			mt_lbprof_printf(MT_LBPROF_LBLOG, "%d:balance fail:cache hot:%d:%d:%s\n", env->dst_cpu, env->src_cpu, p->pid, p->comm);
		return 0;
	}
	return 1;
}
...
redo:
	...
	if (!group) {
		...
		if(mt_lbprof_test(env.fail_reason, MT_LBPROF_HISTORY)){
			mt_lbprof_update_state(1-this_rq->cpu, MT_LBPROF_BALANCE_FAIL_STATE);
		}		
		goto out_balanced;
	}
...
out:
	...
	if( CPU_NEWLY_IDLE == idle){
		mt_lbprof_printf_with_rqinfo(MT_LBPROF_LBLOG, "%d:idle balance:%d:0x%x ", this_cpu, ld_moved, env.fail_reason);
	}else{
		mt_lbprof_printf_with_rqinfo(MT_LBPROF_LBLOG, "%d:periodic balance:%d:0x%x ", this_cpu, ld_moved, env.fail_reason);	
	}
...
void idle_balance(int this_cpu, struct rq *this_rq)
{
	...
	if (this_rq->avg_idle < sysctl_sched_migration_cost){
		mt_lbprof_update_state_has_lock(this_cpu, MT_LBPROF_UPDATE_STATE);
		mt_lbprof_printf_with_rqinfo(MT_LBPROF_LBLOG, "%d:idle balance bypass: %llu %lu ", this_cpu, this_rq->avg_idle, counter);
		return;
	}
