...
static struct i2c_board_info __initdata i2c_stk31xx={ I2C_BOARD_INFO("stk31xx", (0x90>>1))};
#else
...
#if (defined(STK_MANUAL_GREYCARD_CALI) || defined(STK_AUTO_CT_CALI_SATU) || defined(STK_AUTO_CT_CALI_NO_SATU))
static int32_t stk31xx_get_ps_cali_thd(int16_t ps_thd[]);
static void stk31xx_set_ps_thd_to_driver(int16_t ps_thd_data[]);
static int32_t stk31xx_limit_thd_range(int16_t ps_thd_data[]);
static int32_t stk31xx_set_ps_cali(uint8_t force_cali);
...
static int stk31xx_enable_ps(struct i2c_client *client, int enable)
{
	...
	{		
		obj->first_boot = 0;
		err = stk31xx_get_ps_cali_thd(thd_auto);		
		if(err == 0)
		{			
			err = stk31xx_limit_thd_range(thd_auto);
	...
	if(enable)
	{
		if(obj->hw->polling_mode_ps == 0)
		{
			...
			if((err = stk31xx_write_ps(stk31xx_i2c_client, atomic_read(&obj->ps_cmd_val))))
			{
				APS_ERR("write ps: %d\n", err);
				return err;        
			}
		}
		stk31xx_set_ps_cali(0);
		if(obj->hw->polling_mode_ps == 0)
		{
			atomic_set(&obj->ps_cmd_val,  (obj->hw->ps_cmd_val) | 0x02);
			if((err = stk31xx_write_ps(stk31xx_i2c_client, atomic_read(&obj->ps_cmd_val))))
...
static int stk_permission_thread(void *data)
{
	...
	do{
		...
		ret = sys_fchmodat(AT_FDCWD, "/sys/devices/platform/als_ps/driver/pscali" , 0664);
		if(ret < 0)
...
static int32_t stk31xx_get_ps_cali_file(char * r_buf, int8_t buf_size)
{
...
static int32_t stk31xx_set_ps_cali_file(char * w_buf, int8_t w_buf_size)
{
	struct file  *cali_file;
	...
	cali_file = filp_open(STK_CALI_FILE, O_CREAT | O_RDWR | O_SYNC,0666);	
	if(IS_ERR(cali_file))
	{
	...
	else
	{
		...
		ret = sys_fchmodat(AT_FDCWD, "/system/etc/stkpscali.conf" , 0644);
		if(ret < 0)
			printk("fail to execute sys_fchmodat pscali, ret = %d\n", ret);
...
static int32_t stk31xx_get_ps_cali_thd(int16_t ps_thd[])
{
	char r_buf[STK_CALI_FILE_SIZE] = {0};
	int32_t ret;
	stk31xx_obj->cali_file_exist = 0;
	
	if ((ret = stk31xx_get_ps_cali_file(r_buf, STK_CALI_FILE_SIZE)) < 0)
	{		
		return ret;
	}
	else
	{
		if(r_buf[0] == STK_CALI_VER0 && r_buf[1] == STK_CALI_VER1)
		{
			...
			stk31xx_obj->ps_ctk_val = r_buf[4];
			atomic_set(&stk31xx_obj->ps_cali_done, 1);
			stk31xx_obj->cali_file_exist = 1;
		}
		else
		{
...
static void stk31xx_set_ps_thd_to_file(int16_t ps_thd_data[], int16_t ps_ave)
{
	...
	w_buf[4] = ps_ave;			
	stk31xx_set_ps_cali_file(w_buf, sizeof(char)*STK_CALI_FILE_SIZE);			
}
...
static int32_t stk31xx_ct_thd(int16_t ps_stat_data[], int16_t ps_thd_data[])
{
	int16_t CTM, CTA, CTK, offset;
	
	CTA = ps_stat_data[STK_DATA_AVE];
	offset = STK_A_OFFSET;
	if( stk31xx_obj->cali_file_exist == 1 )
	{
		CTK = stk31xx_obj->ps_ctk_val;
		offset = STK_K_OFFSET;
	}
	else
		CTK = STK_DEF_CTK;
	...
	else if(CTA < STK_MAX_PS_CROSSTALK)
	{
		CTM = CTA;
	}
	else
	...
	*/
	...
	stk31xx_limit_thd_range(ps_thd_data);					
...
static int32_t stk31xx_calc_ct_cali_thd(int16_t ps_stat_data[], int16_t ps_thd_data[])
{
...
static int32_t stk31xx_calc_greycard_cali_thd(int16_t ps_stat_data[], int16_t ps_thd_data[])
{	
...
static int32_t stk31xx_judge_grey_card_cali(int16_t ps_stat_data[])
{		
...
static int32_t stk31xx_judge_ct_cali_nosatu(int16_t ps_thd_data[])
{
	int ps_code_low_thd = atomic_read(&stk31xx_obj->ps_low_thd_val);
...
static void stk31xx_set_ps_thd_to_driver(int16_t ps_thd_data[])
{
	...
	atomic_set(&stk31xx_obj->ps_high_thd_val, ps_thd_data[STK_HIGH_THD] ); 
	atomic_set(&stk31xx_obj->ps_low_thd_val, ps_thd_data[STK_LOW_THD] ); 			
	if((ret = stk31xx_write_ps_high_thd(stk31xx_obj->client, ps_thd_data[STK_HIGH_THD])))	
		APS_ERR("%s: write thd failed, err=%d\n", __func__, ret);
	
	if((ret = stk31xx_write_ps_low_thd(stk31xx_obj->client, ps_thd_data[STK_LOW_THD])))	
		APS_ERR("%s: write thd failed, err=%d\n", __func__, ret);		      
}

static int32_t stk31xx_get_several_ps_data(int16_t ps_stat_data[])
{
	u8 ps_data;
	int8_t data_count = 0;	
	...
	old = atomic_read(&stk31xx_obj->ps_cmd_val);
	cur = old & (~SD_PS);   
	
	if(0 == (err = stk31xx_write_ps(stk31xx_obj->client, cur))) 
	{
		atomic_set(&stk31xx_obj->ps_cmd_val, cur);
	}	
	
	sample_time_ps = atomic_read(&stk31xx_obj->ps_cmd_val);
	sample_time_ps&=0x60;
	sample_time_ps>>=0x5;	
	if(sample_time_ps < 4)
	...
	while(data_count < STK_CALI_SAMPLE_NO)
	{
		...
		err = stk31xx_read_ps(stk31xx_obj->client, &ps_data);
...
static int32_t stk31xx_set_ps_cali(uint8_t force_cali)
{
	...
	ret = stk31xx_get_several_ps_data(ps_statistic_data);
	if(ret < 0)
	{
		APS_ERR("%s: stk31xx_get_several_ps_data failed, ret=%d\n", __func__, ret);
		APS_ERR("%s: calibration fail\n", __func__);
		return ret;
	}		
#ifdef STK_AUTO_CT_CALI_SATU
	stk31xx_calc_ct_cali_thd(ps_statistic_data, ps_thd_data);
	stk31xx_ct_thd(ps_statistic_data, ps_thd_data);
	stk31xx_set_ps_thd_to_driver(ps_thd_data);	
	if(force_cali)
		stk31xx_set_ps_thd_to_file(ps_thd_data, ps_statistic_data[STK_DATA_AVE]);	
#elif defined(STK_MANUAL_GREYCARD_CALI)
	if(force_cali)
	{
		ret = stk31xx_judge_grey_card_cali(ps_statistic_data);
		if(ret == 0)
		{
			ret = stk31xx_calc_greycard_cali_thd(ps_statistic_data, ps_thd_data);
			if(ret < 0)
			{					
	...
	stk31xx_calc_ct_cali_thd(ps_statistic_data, ps_thd_data);
	if(stk31xx_judge_ct_cali_nosatu(ps_thd_data) == 0)
	{		
		stk31xx_set_ps_thd_to_driver(ps_thd_data);				
	}					
#endif
...
#if (defined(STK_MANUAL_GREYCARD_CALI) || defined(STK_AUTO_CT_CALI_SATU))
static ssize_t stk31xx_show_pscali(struct device_driver *ddri, char *buf)
{
	int16_t	thd_auto[2] = {125, 110};
	int32_t ret;
	...
	ret = stk31xx_get_ps_cali_thd(thd_auto);		
	if(ret == 0)
	{		
		ret = stk31xx_limit_thd_range(thd_auto);
		if(ret)
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&stk31xx_obj->ps_cali_done)); 
}

static ssize_t stk31xx_store_pscali(struct device_driver *ddri, const char *buf, size_t count)
{
	int value;
	if(!stk31xx_obj)
	{
...
#if (defined(STK_MANUAL_GREYCARD_CALI) || defined(STK_AUTO_CT_CALI_SATU))
static DRIVER_ATTR(pscali, S_IWUSR | S_IRUGO, stk31xx_show_pscali,   stk31xx_store_pscali);
#endif	
...
static int stk31xx_get_ps_value(struct stk31xx_priv *obj, u16 ps)
{
	...
	else
	{
		...
		{
			APS_LOG("obj->ps_high_thd_val= 0x%x,obj->ps_low_thd_val= 0x%x \n", obj->ps_high_thd_val,obj->ps_low_thd_val);
			val = 0;  /*close*/	
		}
		else
		{
