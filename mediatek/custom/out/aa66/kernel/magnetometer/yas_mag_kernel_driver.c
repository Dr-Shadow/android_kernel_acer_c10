...
static struct mutex sensor_data_mutex_yamaha; 
...
extern hwm_sensor_data osensor_data_kernel; 
		...
		case MSENSOR_IOCTL_SENSOR_ENABLE:
			break;
              ...
              case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA: 
                  ...
                  mutex_lock(&sensor_data_mutex_yamaha); 
      
                  osensor_data->values[0] = osensor_data_kernel.values[0]; 
                  osensor_data->values[1] = osensor_data_kernel.values[1]; 
                  osensor_data->values[2] = osensor_data_kernel.values[2]; 
                  osensor_data->status = osensor_data_kernel.status; 
                  //osensor_data->value_divide = CONVERT_O_DIV; 
      
                  mutex_unlock(&sensor_data_mutex_yamaha); 
...
mutex_init(&sensor_data_mutex_yamaha); 
...
{
	MSE_FUN();
	i2c_register_board_info(0, &i2c_yamaha530, 1);
