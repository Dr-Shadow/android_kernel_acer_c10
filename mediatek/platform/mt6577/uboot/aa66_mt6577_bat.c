...
static PMU_ChargerStruct BMT_status;
...
static kal_bool g_bat_full_user_view = KAL_FALSE;
static kal_bool g_Battery_Fail = KAL_FALSE;
static kal_bool batteryBufferFirst = KAL_FALSE;

static int V_PRE2CC_THRES = 3400;
static int V_CC2TOPOFF_THRES = 4050;
static int V_compensate_EVB = 80;
...
static int g_HW_Charging_Done = 0;
static int g_Charging_Over_Time = 0;

static int g_SW_CHR_OUT_EN = 0;
//static int g_bl_on = 1;   //---del by Rachel

static int g_thread_count = 10;

// HW CV algorithm
static unsigned int CHR_CON_0 = 0x7002FA00;
static unsigned int CHR_CON_1 = 0x7002FA04;
static unsigned int CHR_CON_2 = 0x7002FA08;
static unsigned int CHR_CON_4 = 0x7002FA10;
static unsigned int CHR_CON_6 = 0x7002FA18;
static unsigned int CHR_CON_9 = 0x7002FA24;
static unsigned int CHR_CON_10 = 0x7002FA28;
static unsigned int PMIC_RESERVE_CON1 = 0x7002FE84;
static volatile unsigned int save_value = 0x0;
static volatile unsigned int CSDAC_DAT_MAX = 255;
static volatile unsigned int CSDAC_DAT = 0;
static volatile unsigned int VBAT_CV_DET = 0x0;
static volatile unsigned int CS_DET = 0x0;
//int g_sw_cv_enable = 0;

static int CHARGING_FULL_CURRENT = 200;	//green modify, 20121026, change current from 130 to 200

static int g_bat_temperature_pre=0;

static int gADC_BAT_SENSE_temp=0;
static int gADC_I_SENSE_temp=0;
static int gADC_I_SENSE_offset=0;

static kal_uint32 g_eco_version = 0;
#define PMIC6329_E1_CID_CODE    0x0029
static int g_E1_vbat_sense = 0;
 ...
 * EM
****************************************************************************/
static int g_BatteryAverageCurrent = 0;

/*****************************************************************************
 * USB-IF
****************************************************************************/
static int g_usb_state = USB_UNCONFIGURED;
static int g_temp_CC_value = Cust_CC_0MA;

/*****************************************************************************
 * Logging System
****************************************************************************/
static int g_chr_event = 0;
static int bat_volt_cp_flag = 0;
//int Enable_BATDRV_LOG = 0;
static int Enable_BATDRV_LOG = 1;
...
static int prog_first = 1;
static int g_HW_stop_charging = 0;
//static int bl_switch_timer = 0;
static int bat_volt_check_point = 0;
static int getVoltFlag = 0;
static int low_bat_boot_display=0;
//static kal_bool bl_switch = KAL_FALSE;
static kal_bool user_view_flag = KAL_FALSE;
static int charger_ov_boot_display = 0;
...
static void select_charging_curret()
{
...
static void ChargerHwInit(void)
{
    if (Enable_BATDRV_LOG == 1) {
...
static void pchr_turn_off_charging (void)
{
    if (Enable_BATDRV_LOG == 1) {
...
static int BAT_CheckPMUStatusReg(void)
{ 
    if( upmu_is_chr_det() == KAL_TRUE )
    {
...
static int g_Get_I_Charging(void)
{
...
static void BAT_GetVoltage(void)
{ 
...
static void BAT_GetVoltage_notbat(void)
{   
    /* Get V_BAT_SENSE */
    if (g_chr_event == 0) 
    {    	
...
static UINT32 BattVoltToPercent(UINT16 dwVoltage)
{
    UINT32 m = 0;
...
static int BAT_CheckBatteryStatus(void)
{
    ...
    if (low_bat_boot_display == 0)
    {
        ...
        } else if(bat_volt_check_point <= 11) {
            if(mt6577_detect_powerkey() )  //power key press
            {
                leds_battery_low_charging(1);
            }
            else     //power key release
            {
                leds_battery_low_charging(0);
            }
        } else {
...
static PMU_STATUS BAT_ChargingOTAction(void)
{    
...
static PMU_STATUS BAT_PostFullModeAction(void)
{
    if (Enable_BATDRV_LOG == 1) {
...
static void BAT_thread(void)
{
   ...
   // if ( upmu_is_chr_det() == KAL_FALSE )
    ...
    else
    {
        ...
        //---start add by Rachel
        if(BMT_status.total_charging_time >= (MAX_CHARGING_TIME / 2)) //----Rachel charger timer over 12hr, battery error, led blink
        {
        ...
        else
        {
                ...
                && (BMT_status.ICharging <= CHARGING_FULL_CURRENT))
            {
                ...
                BMT_status.bat_full=KAL_TRUE;
                printk("[BATTERY] UI show full first, BMT_status.ICharging:%d, CHARGING_FULL_CURRENT:%d\n", BMT_status.ICharging, CHARGING_FULL_CURRENT);
            }
            else
            {
                post_charging_time=0;
...
void uboot_charging_display()
{
    ...
    {	
		...
		printf("uboot_charging_display-->prog_temp:%d, prog:%d, bat_volt_check_point:%d\n", prog_temp, prog, bat_volt_check_point);
        if(g_bl_on == 1)
        {
#ifdef ANIMATION_NEW	
...
static void check_point_sync_leds(void)
{
...
void aa66_mt65xx_bat_init(void)
{    
	g_R_BAT_SENSE = R_BAT_SENSE;//weli revise for share codebase issue
	g_R_I_SENSE = R_I_SENSE;
	g_R_CHARGER_1 = R_CHARGER_1;
	g_R_CHARGER_2 = R_CHARGER_2;
    printf("********************************************************\n");
    printf("*C10 power off batter init from aa66_mt65xx_bat_init.c* \n");
    printf("********************************************************\n");
    printf("***weli:%d,%d,%d%d************************************\n",g_R_BAT_SENSE,g_R_I_SENSE,g_R_CHARGER_1,g_R_CHARGER_2);
    UINT32 i;
    long tmo;
    long tmo2;
    BOOL checked = FALSE;
    ...
    if ((upmu_is_chr_det() == KAL_TRUE))	
    {
                ...
                if(back_to_charging_animation_flag == false)
		{
                ...
                mt65xx_leds_brightness_set(MT65XX_LED_TYPE_BLUE, LED_OFF);  //---add by Rachel
	     ...
	     //   bl_switch = KAL_FALSE;
            ...
            upmu_chr_baton_tdet_en(1); // sw workaround : 26M off            
             ...
             if (mt6577_detect_key(1) ||  press_pwrkey_count > 0)
             {
                bl_switch = false;
                bl_switch_timer = 0;
                g_bl_on = true;
...
void aa66_mt65xx_bat_init(void)
{
    printf("[BATTERY] Skip AA66 mt65xx_bat_init !!\n\r");
