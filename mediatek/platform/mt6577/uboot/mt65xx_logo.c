...
void cust_show_battery_capacity_new(UINT32 capacity)
{
        ...
        else if (capacity < 25) //12-24
        {
            charging_animation_index++;
            show_logo(3+charging_animation_index); 
            if (charging_animation_index >= 6) charging_animation_index = 0;
        }
        else if (capacity < 40)//20-39
        {
            charging_animation_index++;
            show_logo(4+charging_animation_index); 
            if (charging_animation_index >= 5) charging_animation_index = 0;
        }
        else if (capacity < 60) //40-59
        {
            charging_animation_index++;
            show_logo(5+charging_animation_index); 
            if (charging_animation_index >= 4) charging_animation_index = 0;
        }
        else if (capacity < 80) //60-79
        {
            charging_animation_index++;
            show_logo(6+charging_animation_index); 
            if (charging_animation_index >= 3) charging_animation_index = 0;
        }
        else if (capacity < 100) //80-99
        {
            charging_animation_index++;
            show_logo(7+charging_animation_index); 
            if (charging_animation_index >= 2) charging_animation_index = 0;
        }
        else
        {
            show_logo(9); // battery 100
        }
        mt65xx_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
}
