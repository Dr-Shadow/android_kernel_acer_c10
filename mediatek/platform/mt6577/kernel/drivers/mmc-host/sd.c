...
#define MSDC_MAX_TIMEOUT_RETRY		(2)
...
static u32 msdc_power_tuning(struct msdc_host *host)
{
    ...
    if (mmc_card_sd(card) && (host->id == 1) && (host->power_cycle < MSDC_MAX_POWER_CYCLE) && (host->power_cycle_enable)) {
        // power cycle 
...
#if defined(GPIO_MSDC0_CLK)
	if (msdc0_hw.register_pm!=NULL)    
		{
    	if (!((mt_get_gpio_mode(GPIO_MSDC0_DAT0)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC0_DAT1)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC0_DAT2)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC0_DAT3)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC0_CMD)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC0_CLK)==GPIO_MODE_01)))
    		printk("MSDC0_DAT0<%d> MSDC0_DAT1<%d> MSDC0_DAT2<%d> MSDC0_DAT3<%d> MSDC0_CMD<%d> MSDC0_CLK<%d> \n", mt_get_gpio_mode(GPIO_MSDC0_DAT0) , mt_get_gpio_mode(GPIO_MSDC0_DAT1), mt_get_gpio_mode(GPIO_MSDC0_DAT2), mt_get_gpio_mode(GPIO_MSDC0_DAT3), mt_get_gpio_mode(GPIO_MSDC0_CMD), mt_get_gpio_mode(GPIO_MSDC0_CLK));
    }
#endif
#endif    
#elif defined(CFG_DEV_MSDC1) 
#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT) && (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 1) 
#if defined(GPIO_MSDC1_CLK)
	if (msdc1_hw.register_pm!=NULL)    
		{
    	if (!((mt_get_gpio_mode(GPIO_MSDC1_DAT0)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC1_DAT1)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC1_DAT2)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC1_DAT3)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC1_CMD)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC1_CLK)==GPIO_MODE_01)))
    		printk("MSDC1_DAT0<%d> MSDC1_DAT1<%d> MSDC1_DAT2<%d> MSDC1_DAT3<%d> MSDC1_CMD<%d> MSDC1_CLK<%d> \n", mt_get_gpio_mode(GPIO_MSDC1_DAT0) , mt_get_gpio_mode(GPIO_MSDC1_DAT1), mt_get_gpio_mode(GPIO_MSDC1_DAT2), mt_get_gpio_mode(GPIO_MSDC1_DAT3), mt_get_gpio_mode(GPIO_MSDC1_CMD), mt_get_gpio_mode(GPIO_MSDC1_CLK));
    }
#endif
#endif    
#elif defined(CFG_DEV_MSDC2) 
#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT) && (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 2) 
#if defined(GPIO_MSDC2_CLK)
	if (msdc2_hw.register_pm!=NULL)    
		{
    	if (!((mt_get_gpio_mode(GPIO_MSDC2_DAT0)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC2_DAT1)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC2_DAT2)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC2_DAT3)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC2_CMD)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC2_CLK)==GPIO_MODE_01)))
    		printk("MSDC2_DAT0<%d> MSDC2_DAT1<%d> MSDC2_DAT2<%d> MSDC2_DAT3<%d> MSDC2_CMD<%d> MSDC2_CLK<%d> \n", mt_get_gpio_mode(GPIO_MSDC2_DAT0) , mt_get_gpio_mode(GPIO_MSDC2_DAT1), mt_get_gpio_mode(GPIO_MSDC2_DAT2), mt_get_gpio_mode(GPIO_MSDC2_DAT3), mt_get_gpio_mode(GPIO_MSDC2_CMD), mt_get_gpio_mode(GPIO_MSDC2_CLK));
    }  
#endif
#endif  
#elif defined(CFG_DEV_MSDC3) 
#if defined(CONFIG_MTK_WCN_CMB_SDIO_SLOT) && (CONFIG_MTK_WCN_CMB_SDIO_SLOT == 3) 
#if defined(GPIO_MSDC3_CLK)
	if (msdc3_hw.register_pm!=NULL)    
		{
    	if (!((mt_get_gpio_mode(GPIO_MSDC3_DAT0)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC3_DAT1)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC3_DAT2)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC3_DAT3)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC3_CMD)==GPIO_MODE_01) && (mt_get_gpio_mode(GPIO_MSDC3_CLK)==GPIO_MODE_01)))
    		printk("MSDC3_DAT0<%d> MSDC3_DAT1<%d> MSDC3_DAT2<%d> MSDC3_DAT3<%d> MSDC3_CMD<%d> MSDC3_CLK<%d> \n", mt_get_gpio_mode(GPIO_MSDC3_DAT0) , mt_get_gpio_mode(GPIO_MSDC3_DAT1), mt_get_gpio_mode(GPIO_MSDC3_DAT2), mt_get_gpio_mode(GPIO_MSDC3_DAT3), mt_get_gpio_mode(GPIO_MSDC3_CMD), mt_get_gpio_mode(GPIO_MSDC3_CLK));
    }
#endif
#endif
#endif
...
static int msdc_pio_read(struct msdc_host *host, struct mmc_data *data)
{
	...
	{    	
		...
		if(data->error== -5)  
		{
			ERR_MSG("flags<0x%lx>  cmd_edge<%x>  data_edge<%x>  clk_drv<%x>  cmd_drv<%x>  dat_drv<%x>", host->hw->flags, host->hw->cmd_edge, host->hw->data_edge, host->hw->clk_drv, host->hw->cmd_drv, host->hw->dat_drv);    	
			...
			ERR_MSG("data0<0x%x>  data1<0x%x>  data2<0x%x>  data3<0x%x>",u8_dat0, u8_dat1, u8_dat2, u8_dat3);    
		}
	}
    return data->error;
}
...
static void msdc_dma_start(struct msdc_host *host)
{
    ...
    u32 wints_cmd = MSDC_INT_ACMDRDY | MSDC_INT_ACMDCRCERR | MSDC_INT_ACMDTMO | 
                    MSDC_INT_ACMD19_DONE; 
					 
    sdr_set_bits(MSDC_INTEN, wints | wints_cmd);
    mb();
...
static void msdc_dma_stop(struct msdc_host *host)
{
    ...
    mb();
    sdr_clr_bits(MSDC_INTEN, wints | wints_cmd); /* Not just xfer_comp */
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
    ...
    if(msdc_host_mode[host->id] != mmc->caps)
		{
			...
			{
        		cur_rxdly0 = ((host->hw->dat3rddly & 0x1F) << 24) | ((host->hw->dat2rddly & 0x1F) << 16) |
          	    		     ((host->hw->dat1rddly & 0x1F) << 8)  | ((host->hw->dat0rddly & 0x1F) << 0);
		        cur_rxdly1 = ((host->hw->dat7rddly & 0x1F) << 24) | ((host->hw->dat6rddly & 0x1F) << 16) |
        		  	         ((host->hw->dat5rddly & 0x1F) << 8)  | ((host->hw->dat4rddly & 0x1F) << 0);
		    }
		    sdr_write32(MSDC_DAT_RDDLY0, cur_rxdly0);
		    sdr_write32(MSDC_DAT_RDDLY1, cur_rxdly1); 
#else
    ...
    if (host->mclk != ios->clock) { /* not change when clock Freq. not changed */                
        ...
        if (ios->clock == 0) {
			...
			{
        		cur_rxdly0 = ((hw->dat3rddly & 0x1F) << 24) | ((hw->dat2rddly & 0x1F) << 16) |
          	    		     ((hw->dat1rddly & 0x1F) << 8)  | ((hw->dat0rddly & 0x1F) << 0);
		        cur_rxdly1 = ((hw->dat7rddly & 0x1F) << 24) | ((hw->dat6rddly & 0x1F) << 16) |
        		  	         ((hw->dat5rddly & 0x1F) << 8)  | ((hw->dat4rddly & 0x1F) << 0);
		    }
		    sdr_write32(MSDC_DAT_RDDLY0, cur_rxdly0);
		    sdr_write32(MSDC_DAT_RDDLY1, cur_rxdly1); 
#else
...
int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	msdc_retry(sdc_is_busy(), retry, timeout,host->id);
        ...
        if(ios->signal_voltage == MMC_SIGNAL_VOLTAGE_180){
            if (host->hw->ext_power_switch) {
                host->hw->ext_power_switch(VOL_1800);
            }else{
            	 msdc_sd1_power_switch(1,VOL_1800);
            }
        }else if(ios->signal_voltage == MMC_SIGNAL_VOLTAGE_120){
            if (host->hw->ext_power_switch) {
                host->hw->ext_power_switch(VOL_1200);
            }else{
...
static void msdc_init_hw(struct msdc_host *host)
{
    ...
    INIT_MSG("hw->flags<0x%lx>",hw->flags);
    INIT_MSG("hw->cmd_edge<%x> hw->data_edge<%x>",hw->cmd_edge,hw->data_edge);
    INIT_MSG("hw->clk_drv<%x> hw->cmd_drv<%x> hw->dat_drv<%x>",hw->clk_drv,hw->cmd_drv,hw->dat_drv);
#if 1
    ...
    if (sdr_read32(MSDC_ECO_VER) >= 4) { 
        ...
        if (!is_card_sdio(host)) {  /* internal clock: latch read data, not apply to sdio */           
            ...
            if( MSDC_INTERNAL_CLK == (hw->flags & MSDC_INTERNAL_CLK)) 
            {
							sdr_set_bits(MSDC_PATCH_BIT0, MSDC_PATCH_BIT_CKGEN_CK); 
	    			}
        }
