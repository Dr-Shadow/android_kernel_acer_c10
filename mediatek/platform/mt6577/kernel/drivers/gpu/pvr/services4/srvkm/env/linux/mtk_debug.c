...
static void MTKPPAllocData(MTK_PROC_PRINT_DATA *data, int data_size, int max_line)
{
	...
	data->data = (char *)vmalloc(sizeof(char)*data_size);
	if (data->data == NULL)
	{
	...
	data->line = (char **)vmalloc(sizeof(char*)*max_line);
	if (data->line == NULL)
	{
...
static void* MTKPPSeqStart(struct seq_file *s, loff_t *pos)
{
	...
	if (*pos >= MTK_PP_R_SIZE)	
	{
		...
		//
		// Because B can be the next of A or D (via return NULL), we cannot know what is the last B.
		// But we know that the last A is here, so free the SGXOSTimer data here. 
		//
