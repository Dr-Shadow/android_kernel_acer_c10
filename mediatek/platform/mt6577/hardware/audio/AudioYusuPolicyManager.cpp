                                                  ...
                                                  const char *device_address)
{
    ...
    if (AudioSystem::isOutputDevice(device)) {
        ...
        case AudioSystem::DEVICE_STATE_UNAVAILABLE: {
            ...
            mAvailableOutputDevices = (audio_devices_t)(mAvailableOutputDevices & ~device);
#ifndef ANDROID_DEFAULT_CODE
            ALOGD("setDeviceConnectionState mAvailableOutputDevices = %x",mAvailableOutputDevices);
        ...
        {
            //ALOGD("setDeviceConnectionState add device = %x",device);
...
#ifndef ANDROID_DEFAULT_CODE
            ALOGD("setDeviceConnectionState mAvailableInputDevices = %x",mAvailableInputDevices);
...
void AudioYusuPolicyManager::setForceUse(AudioSystem::force_use usage, AudioSystem::forced_config config)
{
    ...
    for (size_t i = 0; i < mOutputs.size(); i++) {
        ...
        uint32_t delay =0;
        #ifndef ANDROID_DEFAULT_CODE
        if(output == mPrimaryOutput && AudioSystem::isA2dpDevice((AudioSystem::audio_devices)newDevice))
        {
            ssize_t index = mOutputs.indexOfKey(output);
            AudioOutputDescriptor *outputDesc = mOutputs.valueAt(index);
            delay =outputDesc->mLatency*2;
            ALOGD("delay = %d",delay );
        }
        #endif
        setOutputDevice(output, newDevice, (newDevice != 0),delay);
        if (forceVolumeReeval && (newDevice != 0)) {
            applyStreamVolumes(output, newDevice, 0, true);
        }
    }
#ifndef ANDROID_DEFAULT_CODE
    audio_devices_t newDevice = getNewDevice(mPrimaryOutput, true /*fromCache*/);
    if(AudioSystem::popCount(newDevice) != 2 )
    {
        ALOGD("setForceUse newDevice=0x%x config = %x",newDevice,config);
    ...
    {
        ALOGD("setForceUse popCount(newDevice)=2");
        if(usage == AudioSystem::FOR_COMMUNICATION){
          AudioParameter outputCmd = AudioParameter();
          uint32_t value = (uint32_t)usage<<16 |config;
          ALOGD("setForceUse keyAddtForceusePhone, usage=%d, config=%d",usage,config);
          outputCmd.addInt(String8(keyAddtForceusePhone),value);
          mpClientInterface->setParameters(0, outputCmd.toString());
        }
    }
...
{
    ALOGD("setSystemProperty() property %s, value %s", property, value);
...
AudioYusuPolicyManager::IOProfile *AudioYusuPolicyManager::getProfileForDirectOutput(
...
audio_io_handle_t AudioYusuPolicyManager::selectOutput(const SortedVector<audio_io_handle_t>& outputs,
                                                       AudioSystem::output_flags flags)
{
                                             ...
                                             int session)
{
    ...
    if (outputDesc->mRefCount[stream] == 1) {
        ...
        //ALOGD("startOutput() output %d, prevDevice 0x%x, newDevice 0x%x", output, prevDevice, newDevice);
                                            ...
                                            int session)
{
    ...
    if (outputDesc->mRefCount[stream] > 0) {
        ...
        if (outputDesc->mRefCount[stream] == 0) {
          ...
          if((stream==AudioSystem::NOTIFICATION || stream==AudioSystem::RING || stream==AudioSystem::ALARM )&&(mAvailableOutputDevices==7))
            {
                                    ...
                                    AudioSystem::audio_in_acoustics acoustics)
{
    ...
    for(size_t count =0; count <mInputs.size();count++ ){
        ...
        if(inputTemp->mRefCount >0){
            ALOGD("getInput() inputTemp->mRefCount =  %d",inputTemp->mRefCount);
            delete inputDesc;
            return 0;
        }else{
            ALOGD("getInput input can not be used, so release it. mInputs.keyAt(count) = %d",mInputs.keyAt(count));
...
status_t AudioYusuPolicyManager::setStreamVolumeIndex(AudioSystem::stream_type stream,
                                                      int index,
                                                      audio_devices_t device)
{
    ALOGD("VolumeIndex stream = %d index = %d device = 0x%x",stream,index,device);
...
status_t AudioYusuPolicyManager::getStreamVolumeIndex(AudioSystem::stream_type stream,
                                                      int *index,
                                                      audio_devices_t device)
{
    if (index == NULL) {
    ...
    mA2dpSuspended(false), mHasA2dp(false), mHasUsb(false)
{
    ...
    for(int i=0; i < MAX_VOL_CATE; i++)
       for(int k=0;k< MAX_VOL_TYPE ; k++){
           ALOGD("audiovolume_fmr Audio_Custom_Volume.[%d][%d] =%d ",i,k,Audio_Custom_Volume.audiovolume_fmr[i][k]);
    }

    for(int i=0; i < MAX_VOL_CATE; i++)
       for(int k=0;k< MAX_VOL_TYPE ; k++){
           ALOGD("audiovolume_media  Audio_Custom_Volume.[%d][%d] =%d ",i,k,Audio_Custom_Volume.audiovolume_media[i][k]);
    }
    for(int i=0; i < MAX_VOL_CATE; i++)
       for(int k=0;k< MAX_VOL_TYPE ; k++){
           ALOGD("audiovolume_matv  Audio_Custom_Volume.[%d][%d] =%d ",i,k,Audio_Custom_Volume.audiovolume_matv[i][k]);
    }
    for(int i=0; i < MAX_VOL_CATE; i++)
        for(int k=0;k< MAX_VOL_TYPE ; k++){
        ALOGD("audiovolume_key  Audio_Custom_Volume.[%d][%d] =%d ",i,k,Audio_Custom_Volume.audiovolume_key[i][k]);
    }
#endif
    ...
    mAvailableOutputDevices = (audio_devices_t)(AudioSystem::DEVICE_OUT_EARPIECE | AudioSystem::DEVICE_OUT_SPEAKER);
    mAvailableInputDevices = (audio_devices_t)AudioSystem::DEVICE_IN_BUILTIN_MIC;
#endif
...
status_t AudioYusuPolicyManager::checkOutputsForDevice(audio_devices_t device,
                                                       AudioSystem::device_connection_state state,
                                                       SortedVector<audio_io_handle_t>& outputs)
{
...
void AudioYusuPolicyManager::closeOutput(audio_io_handle_t output)
{
...
SortedVector<audio_io_handle_t> AudioYusuPolicyManager::getOutputsForDevice(audio_devices_t device)
{
    SortedVector<audio_io_handle_t> outputs;
...
bool AudioYusuPolicyManager::vectorsEqual(SortedVector<audio_io_handle_t>& outputs1,
...
// Move effects associated to this strategy from previous output to new output
        for (size_t i = 0; i < mEffects.size(); i++) {
            ...
            if (desc->mSession != AudioSystem::SESSION_OUTPUT_STAGE &&
                    ...
                    desc->mIo == srcOutput) {
                ALOGD("checkOutputForStrategy() moving effect %d to output %d", mEffects.keyAt(i), dstOutput);
...
audio_io_handle_t AudioYusuPolicyManager::getA2dpOutput()
{
    if (!mHasA2dp) {
        return 0;
    }
...
audio_devices_t AudioYusuPolicyManager::getNewDevice(audio_io_handle_t output, bool fromCache)
{
...
audio_devices_t AudioYusuPolicyManager::getDevicesForStream(AudioSystem::stream_type stream) {
...
void AudioYusuPolicyManager::handleNotificationRoutingForStream(AudioSystem::stream_type stream) {
    switch(stream) {
    case AudioSystem::MUSIC:
        checkOutputForStrategy(STRATEGY_SONIFICATION_RESPECTFUL);
        updateDeviceForStrategy();
...
audio_devices_t AudioYusuPolicyManager::getDeviceForStrategy(routing_strategy strategy,
                                                             bool fromCache)
{
    uint32_t device = 0;
    ...
    case STRATEGY_MEDIA: {
        ...
        if (mHasA2dp && (mForceUse[AudioSystem::FOR_MEDIA] != AudioSystem::FORCE_NO_BT_A2DP) &&
            ...
            if (device2 == 0) {
             ...
             if((STRATEGY_SONIFICATION==strategy)&&(mAvailableOutputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET))
            {
                 device2=0;
             }
        }
...
uint32_t AudioYusuPolicyManager::checkDeviceMuteStrategies(AudioOutputDescriptor *outputDesc,
...
#ifndef ANDROID_DEFAULT_CODE
    //headphone can't be closed ,customer will caught this action,so do not do tempmute for headphone
    bool tempMute = (outputDesc->refCount() != 0) && (device != prevDevice) && \
                    (!(device & (AUDIO_DEVICE_OUT_WIRED_HEADPHONE | AUDIO_DEVICE_OUT_WIRED_HEADSET)));
#else
    bool tempMute = (outputDesc->refCount() != 0) && (device != prevDevice);
#endif
    #ifndef ANDROID_DEFAULT_CODE
    ...
    for (size_t i = 0; i < NUM_STRATEGIES; i++) {
        ...
        if (doMute || tempMute) {
            for (size_t j = 0; j < mOutputs.size(); j++) {
                ...
                if((STRATEGY_PROPRIETARY == i) && (mute == false)){ // fix issue: headphone pluggin, fm play, enter setting, play ringtone, stop ringtone, fm sound will leak from speaker
...
uint32_t AudioYusuPolicyManager::setOutputDevice(audio_io_handle_t output,
                                             audio_devices_t device,
                                             bool force,
                                             int delayMs)
{
    ...
    ALOGD("setOutputDevice() output %d device %04x delayMs %d prevDevice %04x delayMs = %d",
        output, device, delayMs,prevDevice,delayMs);
    if (device != 0) {
        outputDesc->mDevice = device;
    }
...
#ifndef ANDROID_DEFAULT_CODE
    ALOGW_IF(AudioSystem::popCount(device)>2,"setOutputDevice() multi devices=0x%x",device);
  //  if(output == mPrimaryOutput && AudioSystem::popCount(device) == 2)
    if(output == mPrimaryOutput && ((AudioSystem::popCount(device) == 2)||((mAvailableOutputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET)&&(device== AudioSystem::DEVICE_OUT_SPEAKER))))
  {
        tempflag &=(0<<0);
         tempflag |=(1<<1);
        // wait for one more latency for  the PCM output buffers to empty
        if(device != prevDevice)
        {
            usleep(outputDesc->mLatency*1000);
        }
        AudioParameter outputCmd = AudioParameter();
 ...
 #ifndef ANDROID_DEFAULT_CODE
   // if(output == mPrimaryOutput && AudioSystem::popCount(prevDevice) == 2 && AudioSystem::popCount(device)!= 2)
     if(output == mPrimaryOutput && ((AudioSystem::popCount(prevDevice) == 2 && AudioSystem::popCount(device)!= 2)||tempflag==3))
   {
          tempflag=0;  
        //force hardware to use None
        AudioParameter outputCmd = AudioParameter();
...
AudioYusuPolicyManager::IOProfile *AudioYusuPolicyManager::getInputProfile(audio_devices_t device,
                                                   uint32_t samplingRate,
                                                   uint32_t format,
                                                   uint32_t channelMask)
{
...
audio_devices_t AudioYusuPolicyManager::getDeviceForInputSource(int inputSource)
{
    uint32_t device = 0;
...
audio_devices_t AudioYusuPolicyManager::getDeviceForVolume(audio_devices_t device)
{
    if (device == 0) {
...
AudioYusuPolicyManager::device_category AudioYusuPolicyManager::getDeviceCategory(audio_devices_t device)
{
...
float AudioYusuPolicyManager::volIndexToAmpl(audio_devices_t device, const StreamDescriptor& streamDesc,
        int indexInUi)
{
...
const AudioYusuPolicyManager::VolumeCurvePoint
    AudioYusuPolicyManager::sDefaultSystemVolumeCurve[AudioYusuPolicyManager::VOLCNT] = {
    {1, -24.0f}, {33, -18.0f}, {66, -12.0f}, {100, -6.0f}
};

const AudioYusuPolicyManager::VolumeCurvePoint
    AudioYusuPolicyManager::sHeadsetSystemVolumeCurve[AudioYusuPolicyManager::VOLCNT] = {
    {1, -30.0f}, {33, -26.0f}, {66, -22.0f}, {100, -18.0f}
};

const AudioYusuPolicyManager::VolumeCurvePoint
            *AudioYusuPolicyManager::sVolumeProfiles[AUDIO_STREAM_CNT]
...
float AudioYusuPolicyManager::computeVolume(int stream,
                                            int index,
                                            ...
                                            audio_devices_t device)
{
    ...
    if ((device & (AudioSystem::DEVICE_OUT_BLUETOOTH_A2DP |
        ...
        // just stopped
        #ifndef ANDROID_DEFAULT_CODE
        uint32 mask = (uint32)(device)&(uint32)(~AUDIO_DEVICE_OUT_SPEAKER);
...
status_t AudioYusuPolicyManager::checkAndSetVolume(int stream,
                                                   int index,
                                                   audio_io_handle_t output,
                                                   audio_devices_t device,
                                                   int delayMs,
...
#ifdef MATV_AUDIO_LINEIN_PATH
            if((stream==AudioSystem::MATV)&&(output!=mPrimaryOutput)){
                bSetStreamVolume=0; //MATV with line-in path is only ouput from primaryoutput
            }
#endif
#ifdef FM_ANALOG_IN_SUPPORT
            if((stream==AudioSystem::FM)&&(output!=mPrimaryOutput)){
...
void AudioYusuPolicyManager::applyStreamVolumes(audio_io_handle_t output,
                                                audio_devices_t device,
                                                int delayMs,
                                                bool force)
{
...
void AudioYusuPolicyManager::setStrategyMute(routing_strategy strategy,
                                             bool on,
                                             audio_io_handle_t output,
                                             int delayMs,
                                             audio_devices_t device)
...
void AudioYusuPolicyManager::setStreamMute(int stream,
                                           bool on,
                                           audio_io_handle_t output,
                                           int delayMs,
                                           audio_devices_t device)
...
bool AudioYusuPolicyManager::needsDirectOuput(audio_stream_type_t stream,
                                              uint32_t samplingRate,
...
audio_devices_t AudioYusuPolicyManager::AudioOutputDescriptor::device()
{
    if (isDuplicated()) {
...
uint32_t AudioYusuPolicyManager::AudioOutputDescriptor::latency()
{
    if (isDuplicated()) {
...
bool AudioYusuPolicyManager::AudioOutputDescriptor::sharesHwModuleWith(
...
audio_devices_t AudioYusuPolicyManager::AudioOutputDescriptor::supportedDevices()
{
    if (isDuplicated()) {
...
AudioYusuPolicyManager::AudioInputDescriptor::AudioInputDescriptor(const IOProfile *profile)
...
AudioYusuPolicyManager::StreamDescriptor::StreamDescriptor()
...
int AudioYusuPolicyManager::StreamDescriptor::getVolumeIndex(audio_devices_t device)
{
    device = AudioYusuPolicyManager::getDeviceForVolume(device);
...
void AudioYusuPolicyManager::StreamDescriptor::dump(int fd)
{
    const size_t SIZE = 256;
    char buffer[SIZE];
    String8 result;
...
AudioYusuPolicyManager::HwModule::HwModule(const char *name)
    : mName(strndup(name, AUDIO_HARDWARE_MODULE_ID_MAX_LEN)), mHandle(0)
{
}

AudioYusuPolicyManager::HwModule::~HwModule()
{
...
void AudioYusuPolicyManager::HwModule::dump(int fd)
{
    const size_t SIZE = 256;
    char buffer[SIZE];
    String8 result;
...
AudioYusuPolicyManager::IOProfile::IOProfile(HwModule *module)
    : mFlags((audio_output_flags_t)0), mModule(module)
{
}

AudioYusuPolicyManager::IOProfile::~IOProfile()
{
}
...
bool AudioYusuPolicyManager::IOProfile::isCompatibleProfile(audio_devices_t device,
                                                            uint32_t samplingRate,
                                                            uint32_t format,
                                                            uint32_t channelMask,
                                                            audio_output_flags_t flags) const
{
    ...
    if (samplingRate != 0) {
#else
    if ((samplingRate != 0)&&((device&AUDIO_DEVICE_IN_ALL || device&AUDIO_DEVICE_IN_FM))==0) {
#endif
        size_t i;
...
void AudioYusuPolicyManager::IOProfile::dump(int fd)
{
    const size_t SIZE = 256;
    char buffer[SIZE];
    String8 result;
...
uint32_t AudioYusuPolicyManager::stringToEnum(const struct StringToEnum *table,
                                              size_t size,
                                              const char *name)
{
...
audio_output_flags_t AudioYusuPolicyManager::parseFlagNames(char *name)
{
    uint32_t flag = 0;
...
audio_devices_t AudioYusuPolicyManager::parseDeviceNames(char *name)
{
    uint32_t device = 0;
...
void AudioYusuPolicyManager::loadSamplingRates(char *name, IOProfile *profile)
{
...
void AudioYusuPolicyManager::loadFormats(char *name, IOProfile *profile)
{
    char *str = strtok(name, "|");
...
void AudioYusuPolicyManager::loadInChannels(char *name, IOProfile *profile)
{
...
void AudioYusuPolicyManager::loadOutChannels(char *name, IOProfile *profile)
{
    const char *str = strtok(name, "|");
...
status_t AudioYusuPolicyManager::loadInput(cnode *root, HwModule *module)
{
...
status_t AudioYusuPolicyManager::loadOutput(cnode *root, HwModule *module)
{
    cnode *node = root->first_child;
...
void AudioYusuPolicyManager::loadHwModule(cnode *root)
{
...
void AudioYusuPolicyManager::loadHwModules(cnode *root)
{
...
void AudioYusuPolicyManager::loadGlobalConfig(cnode *root)
{
...
status_t AudioYusuPolicyManager::loadAudioPolicyConfig(const char *path)
{
    cnode *root;
    char *data;
...
void AudioYusuPolicyManager::defaultAudioPolicyConfig(void)
{
    HwModule *module;
    IOProfile *profile;
    ...
    profile->mSupportedDevices =
        audio_devices_t(AUDIO_DEVICE_OUT_EARPIECE |
            ...
            AUDIO_DEVICE_OUT_HDMI_AUD |   // not ANDROID_DEFAULT_CODE
...
#ifndef ANDROID_DEFAULT_CODE
    profile->mSupportedDevices =
    (audio_devices_t)(AUDIO_DEVICE_IN_COMMUNICATION |
            ...
            AUDIO_DEVICE_IN_FM |
            AUDIO_DEVICE_IN_DEFAULT) ;// not ANDROID_DEFAULT_CODE
#endif
    module->mInputProfiles.add(profile);
...
#ifdef MTK_AUDIO_GAIN_TABLE
void AudioYusuPolicyManager::updateAnalogVolume(audio_io_handle_t output, audio_devices_t device,int delayMs)
{
    if(output == mPrimaryOutput)
    {
        if (device == 0) {
...
{
    ALOGD("SetPolicyManagerParameters par1 = %d par2 = %d par3 = %d par4 = %d",par1,par2,par3,par4);
    audio_devices_t device ;
    ...
    status_t volStatus =checkAndSetVolume((int)AudioSystem::FM, mStreams[AudioSystem::FM].getVolumeIndex(device),mPrimaryOutput,device,50,true);
...
{
    ALOGD("LoadCustomVolume Audio_Custom_Volume");
...
{
    //ALOGD("Audio_Find_Normal_Output_Device mRoutes = %x",mRoutes);
...
{
     //ALOGD("Audio_Find_Incall_Output_Device mRoutes = %x",mRoutes);
...
float AudioYusuPolicyManager::computeCustomVolume(int stream, float &volInt)
{
    ...
    //ALOGD("stream = %d after computeCustomVolume , volInt = %d volume = %f",stream,volInt,volume);
    return volume;
}
...
int AudioYusuPolicyManager::GetOutputDevice()
{
    ...
    if(mPhoneState == AudioSystem::MODE_RINGTONE){
        ...
        //ALOGV("mForceUse[AudioSystem::FOR_COMMUNICATION] = %d",mForceUse[AudioSystem::FOR_COMMUNICATION]);
