...
void AudioYusuVolumeController::ApplyGain(uint16 DegradedBGain,uint32 mode,uint32 routes)
{
   ...
   if(mode ==  1 && routes == 1 && mAudioHardware->IsHeadsetBTConnect() == false){
        char value[5];
        int audioGain;

        property_get("streamin.audiogain", value, "0");
        audioGain = atoi(value);
        ALOGD("Re-ApplyGain for ringtone to %d DB", audioGain);
        
        if(audioGain <= 3 || audioGain >= 45)
            mAnaReg->SetAnaReg(AUDIO_CON1, 14, 0x003f); 
        else
            mAnaReg->SetAnaReg(AUDIO_CON1, audioGain, 0x003f);
    }
...
{
    int MapVolume;
    LOG_VOLUMECONTROLLER("setMasterVolume = %f, mode=%d, routes=%d",volume, mode,routes);
    mVolume = volume;
