...
int ensure_path_mounted(const char* path) {
    ...
    if (!mv) {
	    result = mount("/emmc@usrdata", "/data", "ext4",
...
{	
    ...
    char buff[4096]  = {0};
    int n;
    LOGD("fopen rb %s , wb %s end\n", in, out);

    fd=fopen( out, "wb+" );

    if(0 == fd){
      LOGE("out filename = %s fopen error %s\n",out,strerror(errno));
      return 0;
    }
    fs=fopen( in, "rb" );
    if(0 == fs){

      LOGE("in filename = %s fopen error %s\n",in,strerror(errno));
      return 0;
    }
    while( 1 )
    {
...
{
		int i = 0;
                printf("restoreapp start");	
		FILE *fp = fopen("/system/app/.restore_list", "r");
		...
		{	
			...
			while(!feof(fp))
			{
				...
				{				
					...
					if(i==2)
					{
						sprintf(buffer, "/data/app%s", name);						
						copy(tmp, buffer);						
					}
				}
				i=0;	
	  ...
	  if (result != 0) {	
		LOGE("format_volume: make_extf4fs failed on /emmc@usrdata\n");
	    	return -1;
             ...
             result = mkdir("/data/app",S_IRUSR|S_IWUSR);

             copy("/system/etc/.keep_list", "/data/app/.keep_list");
             copy("/system/etc/.restore_list", "/data/app/.restore_list");  
             int err = execl("/system/bin/restoreapp", "restoreapp", NULL);
             exit(-3);
          }
        }	
...
int clear_emmc_entry()
{
    ...
    //copy("/data/app/.keep_list", "/system/etc/.keep_list");
    //copy("/data/app/.restore_list", "/system/etc/.restore_list");   
