...
U32 pmic6329_i2c_read (U8 chip, U8 *cmdBuffer, int cmdBufferLen, U8 *dataBuffer, int dataBufferLen)
{
    ...
    ret_code = i2c_v1_write(I2C2, chip, cmdBuffer, cmdBufferLen);    // set register command
    if (ret_code != I2C_OK)
        return ret_code;

    ret_code = i2c_v1_read(I2C2, chip, dataBuffer, dataBufferLen);
...
U32 pmic6329_i2c_write (U8 chip, U8 *cmdBuffer, int cmdBufferLen, U8 *dataBuffer, int dataBufferLen)
{
    ...
    ret_code = i2c_v1_write(I2C2, chip, write_data, transfer_len);
...
kal_uint32 pmic_read_interface (kal_uint8 RegNum, kal_uint32 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
...
kal_uint32 pmic_config_interface (kal_uint8 RegNum, kal_uint32 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    U8 chip_slave_address = 0xC0;
    U8 cmd = 0x0;
    int cmd_len = 1;
...
kal_uint32 pmic_bank1_read_interface (kal_uint8 RegNum, kal_uint32 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
...
kal_uint32 pmic_bank1_config_interface (kal_uint8 RegNum, kal_uint32 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    U8 chip_slave_address = 0xC2;
    U8 cmd = 0x0;
    int cmd_len = 1;
...
U32 pmic6329_get_version (void)
{
    ...
    {
        printf("[LK_pmic6329_get_version] PMIC6329_E1_CID_CODE !!\n");
    }
	else if (eco_version == PMIC6329_E2_CID_CODE)
    {
        printf("[LK_pmic6329_get_version] PMIC6329_E2_CID_CODE !!\n");
    } 
    else
    {
        printf("[LK_pmic6329_get_version] UNKNOWN VERSION !!\n");
...
U32 pmic_IsUsbCableIn (void) 
{
    ...
    if (data & (BANK_0_PCHR_CHRDET_MASK << BANK_0_PCHR_CHRDET_SHIFT)){
        ...
        printf("[LK_PMIC_IsUsbCableIn] PMIC_CHRDET_EXIST\n");
        ret_code = PMIC_CHRDET_EXIST;
    }else{
        //printf("[PMIC_IsUsbCableIn] Get register %x : data = %x\n", cmd, data);
        printf("[LK_PMIC_IsUsbCableIn] PMIC_CHRDET_NOT_EXIST\n");
...
void PMIC_CUSTOM_SETTING_V1(void)
{
	...
	//enable vgp2 for tp effect i2c issue
...
int mt6329_detect_powerkey(void)
{
	U32 ret_code = I2C_OK;
	U8 chip_slave_address = mt6329_BANK0_SLAVE_ADDR_WRITE;
    U8 cmd = 0x09;
	...
	if (data & (BANK_0_PWRKEY_DEB_MASK << BANK_0_PWRKEY_DEB_SHIFT)){     
        printf("[mt6329_detect_powerkey] Release\n");
        //return 1;
        return 0;
    }else{
        printf("[mt6329_detect_powerkey] Press\n");
        //return 0;
        return 1;
    }
}

int mt6329_detect_homekey(void)
{
	U32 ret_code = I2C_OK;
	U8 chip_slave_address = mt6329_BANK0_SLAVE_ADDR_WRITE;
    U8 cmd = 0x0C;
	...
	if (data & (BANK_0_HOMEKEY_DEB_MASK << BANK_0_HOMEKEY_DEB_SHIFT)){     
        printf("[mt6329_detect_homekey] Release\n");
        return 0;
    }else{
        printf("[mt6329_detect_homekey] Press\n");
        return 1;
    }
}
...
//==============================================================================
// PMIC6329 API for LK : Get CHRDET status
//==============================================================================
...
//[**start add  by Rachel, 2012.10.29, add vibrator to lk**]
void cci_enable_vibrator(void)
{
	kal_uint32 ret=0;
	pmic_config_interface( (kal_uint8)BANK0_DIGLDO_CON33,(kal_uint8)6,0X7,0X4);//SET VIBR VOLTAGE
	ret=pmic_config_interface( (kal_uint8)BANK0_DIGLDO_CON33,0x1,0x1,0x0);//ENABLE VIBR
}
void cci_disable_vibrator(void)
{
	kal_uint32 ret=0;

	ret=pmic_config_interface( (kal_uint8)BANK0_DIGLDO_CON33,0x0,0x1,0x0);//DISABLE VIBR
}
//[**end add  by Rachel, 2012.10.29, add vibrator to lk**]
